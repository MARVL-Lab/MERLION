...
def main(video_frame_path, save_selected_to, threshold, prompt_file):
    global long_text
    ...
    # Save config
    os.makedirs(save_selected_to, exist_ok=True)
    with open(os.path.join(save_selected_to, "prompts_used.txt"), "w+") as pfile:
        for p in long_text:
            pfile.write(f"{p}\n")
    with open(os.path.join(save_selected_to, "args.txt"), "w+") as afile:
        json.dump(vars(args), afile, indent=4)


    # Selected images output dir
    selected_images_dir = os.path.join(save_selected_to, "selected_images")
    ...
    # Run MERLION pre-sampling for each image
    for frame_id, frame_path in enumerate(tqdm(video_frame_path_list)):
        ...
        if _sim[0] > threshold:
            src_path = os.path.join(video_frame_path, frame_path)
            dest_path = os.path.join(selected_images_dir, frame_path)
            shutil.copyfile(src_path, dest_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MERLION video frame summarization presampling.")
    ...
    parser.add_argument("--save_selected_to", type=str, required=True, help="Directory to save the presampled images.")
    parser.add_argument("--threshold", type=float, default=0.50, help="Threshold for pre-sampling frames.")

    args = parser.parse_args()

    start_time = time.time()
    main(args.video_frame_path, args.save_selected_to, args.threshold, args.prompt_file)
